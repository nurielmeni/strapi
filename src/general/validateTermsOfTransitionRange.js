...
const validTermsOfTransitionRange = (tot, rangeBegin, rangeEnd) => {
  ...
  for (let i = 0; i < ranges.length - 1; i++) {
    if (ranges[i].end !== ranges[i + 1].start - 1) {
      throw new Error(
        `[${tot.name
        }] Bad ranges: The terms of transition must cover the whole range: ${rangeBegin} to ${rangeEnd}. Missing: ${ranges[i].end + 1
        } - ${ranges[i + 1].start - 1}`
      );
    }
  }
...
function validateTermsOfTransitionRange (
  tot,
  rangeBegin = 0,
  rangeEnd = 100
) {
  if (!tot || !Array.isArray(tot) || tot.length === 0) return new ValidationObj();
