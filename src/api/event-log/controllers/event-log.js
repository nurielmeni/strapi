 ...
 */
  ...
  async create (ctx) {
    ...
    if (!eventLogType)
      return ctx.forbidden(
        'An event log must have a valid Event Log Type(event_log_type)'
      );
  ...
  async find (ctx) {
    ...
    return entities.map((entity) =>
      sanitizeEntity(entity, { model: strapi.models['event-log'] })
    );
  },

  async userTimeByDays (ctx) {
    const { id: userId } = ctx.params;
    if (!userId) return ctx.forbidden('An event query must have a valid User');
    ...
    for (const entity of entities) {
      const {
        event_log_type: { event_type: eventType },
        time
      } = entity;
